...
uint8_t day; //global time and date vars
uint8_t month;
uint8_t year;
uint8_t hours;
uint8_t mins;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if(GPIO_Pin == KEY_BUTTON_PIN)  //GPIO_PIN_0 USER BUTTOn
  {
		...
		HAL_RTC_GetTime(&RTCHandle,&RTC_TimeStructure,RTC_FORMAT_BIN); //get time and date
		HAL_RTC_GetDate(&RTCHandle,&RTC_DateStructure,RTC_FORMAT_BIN);
		uint8_t h=RTC_TimeStructure.Hours;
		uint8_t m=RTC_TimeStructure.Minutes;
		uint8_t s=RTC_TimeStructure.Seconds;
		
		uint8_t AA[3]={h,m,s}; //store current time in an array
		uint8_t * bufferdata=(uint8_t *)AA;	
		
		I2C_BufferWrite(&I2c3_Handle,EEPROM_ADDRESS, memLocation+j*3, bufferdata, 3); //write current time to eeprom
		j++;

		I2C_BufferWrite(&I2c3_Handle,EEPROM_ADDRESS, memLocation, &j, 1); //write mem address of latest time to eeprom
		
		//LCD_DisplayInt(15,1,j);
		
		if (state==1){

			if (cursor==0) { //increment day
				//offset+=1;
				day+=1;
				if(day > 31){
					day = 1;
			...
			else if (cursor==1) { //if cursor is incremented, increment month
				month++;
				//month = month%12;
				if(month > 12){
					month = 1;
			...
			else if (cursor==2) { //increment year
				year++;
				if(year > 99){
					year = 00;
				}
			...
			else if (cursor==3) { //increment hours
				hours++;
				if(hours > 24){
					hours = 0;
				}
			...
			else if (cursor==4) { //increment minutes
				mins++;
				if(mins > 60){
					mins = 0;
				}
			...
			else if (cursor==5) { //increment seconds
				seconds++;
				if(seconds > 60){
					seconds = 0;
				}
	...
	if(GPIO_Pin == GPIO_PIN_1) { //PC1
		if (state==0){state=1;} //go to settings state
		
		else if (state==1) {
			if (pause==1){ //increment cursor if button pressed again
				cursor++;
				
				if (cursor==0){ //
					LCD_DisplayString(5,1,"  "); //initializes cursor, shows location
			
				}
				else if (cursor==1){
					LCD_DisplayInt(5,1,day);
					LCD_DisplayString(5,4,"  ");//day
				}
				
				else if (cursor==2){
					LCD_DisplayInt(5,4,month);
					LCD_DisplayString(5,7,"  ");//month
				}
				
				else if (cursor==3){
					LCD_DisplayInt(5,7,year);
					LCD_DisplayString(8,1,"  ");//year
				}
				
				else if (cursor==4){
					LCD_DisplayInt(8,1,hours);
					LCD_DisplayString(8,4,"  ");//hours
				}
				
				else if (cursor==5){
					LCD_DisplayInt(8,4,mins);
					LCD_DisplayString(8,7,"  ");//mins
				}
				
				else if (cursor==6){
					LCD_DisplayInt(8,7,seconds); //seconds
					cursor=0;
  ...
  {
		if (state==0){
			state=3; //show 2 latest times
			LCD_DisplayString(8,1,"Latest Time:");
			uint8_t readData=0x00;
			for (int i=0;i<3;i++){ //reading data from eeprom
			...
			for (int k=0;k<3;k++){ //reading data from eeprom
				LCD_DisplayInt(12,1+(3*k), (I2C_ByteRead(&I2c3_Handle,EEPROM_ADDRESS, memLocation+3*j-6+k))); //display time
				if (k!=2){LCD_DisplayString(12,1+(3*k+2),":");}
			}
			
		}
		else if (state==1){ //settings state
			RTC_TimeStructure.Hours=hours; //store changed time
		...
		else if (state==3){ //get rid of 2 most recent times on screen
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	if (state==0){
		...
		LCD_DisplayString(4,1,"HH:MM:SS"); //display time
		...
		LCD_DisplayString(1,1,"DD:MM:YY"); //display date
		...
		if (BSP_PB_GetState(BUTTON_KEY)!=1){ //only display date if user is pressed
		...
		if (pause==0){
				BSP_LCD_Clear(LCD_COLOR_WHITE);
				HAL_RTC_GetTime(&RTCHandle,&RTC_TimeStructure,RTC_FORMAT_BIN); //get current time
				...
				//BSP_LCD_ClearStringLine(5);
				LCD_DisplayString(1,0,"Setting the"); //date and time settings menu
				...
				seconds = RTC_TimeStructure.Seconds; //storing time locally
			...
			//HAL_RTC_GetTime(&RTCHandle,&RTC_TimeStructure,RTC_FORMAT_BIN);
			//HAL_RTC_GetDate(&RTCHandle,&RTC_DateStructure,RTC_FORMAT_BIN);
			LCD_DisplayString(4,1,"HH:MM:SS"); //display most recent times
			LCD_DisplayInt(5,1,RTC_TimeStructure.Hours);
			LCD_DisplayString(5,3,":");
			LCD_DisplayInt(5,4,RTC_TimeStructure.Minutes);
			LCD_DisplayString(5,6,":");
