...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if(GPIO_Pin == KEY_BUTTON_PIN)  //GPIO_PIN_0 USER BUTTOn
  {
		...
		if (state==1){
			...
			if (cursor==0) {
				offset+=1;
				LCD_DisplayInt(5,1,day+offset);
			
			}
			else if (cursor==1) {
				offset+=1;
				LCD_DisplayInt(5,4,month+offset);
			
			}
			else if (cursor==2) {
				offset+=1;
				LCD_DisplayInt(5,7,year+offset);
			
			}
			else if (cursor==3) {
				offset+=1;
				LCD_DisplayInt(8,1,hours+offset);
			
			}
			else if (cursor==4) {
				offset+=1;
				LCD_DisplayInt(8,4,mins+offset);
			
			}
			else if (cursor==5) {
				offset+=1;
				LCD_DisplayInt(8,7,seconds+offset);
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	if (state==0){
		...
		if (pause==0){ // if the time is not paused, get an updated time then pause any updates
